...
class MixerFP { 
    float pr, rate; // final prediction, learning rate for update
    float *wx,*tx; //N*M weights, N inputs
    int cxt,t;     // context, number of inputs in t, 0 to N
    const int M,N; // max inputs, max contexts
public:  
    MixerFP(int n, int m): pr(0.5),rate(0.28955),M(m),N(n),t(0) {
        alloc(wx, N*M);
        alloc(tx, N);
        for (int i=0; i<N; i++) tx[i]=0.5f;                        // initial prediction
        for (int i=0; i<4096; i++) stretchf[i]=stretch(i)/4095.0f; // precalculate stretch table
        for (int i=0; i<N*M; i++) wx[i]=1.5f;                      // initial weight
    }
    ~MixerFP(){
        ...
        for (int i=0; i<N; ++i) {
            float w=0;
            for (int j=0; j<M; ++j)
                w+=wx[j*N+i];//,printf("%0.5f ",wx[j*N+i]);;
            printf("%0.5f ", w/M);
        }
        printf("\n"); */
    }
    ...
    void update(int y) {
        float err = rate * (pr - y);
        // skip error if below treshold
        if(err>-0.00044f && err<0.00024f) return;
        // update wx
        float *w= &wx[cxt*N];
        for (int i=0; i<N; i++)
            w[i]-=err*tx[i]; 
